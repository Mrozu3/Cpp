...
bool Insert(int x, position p, List &l)
{
    ...
    if( p < 0 || p > END(l) || END(l) >= maxlength) { return false; } //Jesli sie nie powiodlo 
...
//Duplikuje wszystkie elementy w liscie
position Duplicate(List &l) {
    int temp = END(l); //zapisujemy obecny koniec listy
    //Dopoki nie dojdziey do zapisanego konca listy
    for ( int i = 0; i < temp; i++) {
            Insert(l.elements[i],END(l),l); //Dodawaj elementy z poczatku na koniec   
            }
    }

//Usuwa wszystkie duplikaty z listy
position Purge(List &l) {
    //Petla w petli, bierzemy pierwszy element i porownujemy go z nastepnym
    for( int i = 0; i < END(l); i++) {
          for( int j = i+1; j < END(l); j++) {
              //std::cout<<" Porownuje "<<i<<" z "<<j<<std::endl;
              if(l.elements[i] == l.elements[j] ) {         //Jesli sprawdzany przez nas element znajduje sie na liscie drugi raz
                  Delete(j,l); //Usuwamy go
                  j = j-1; //Sprawdzamy jeszcze raz ten element na liscie
              }
          }
    }
}
...
Duplicate(l);
print(l);
Insert(2,Next(Locate(100,l),l),l);
print(l);
Insert(2,Next(Locate(0,l),l),l);
print(l);
Duplicate(l);
print(l);
Purge(l);
